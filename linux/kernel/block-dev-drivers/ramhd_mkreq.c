...
#define RAMHD_MAX_DEVICES       2
#define RAMHD_MAX_PARTITIONS    4
#define RAMHD_SECTOR_SIZE       512
#define RAMHD_SECTORS           16
#define RAMHD_HEADS             4
#define RAMHS_CYLINDERS         256

#define RAMHD_SECTOR_TOTAL      (RAMHD_SECTORS * RAMHD_HEADS * RAMHS_CYLINDERS)
...
static char *sdisk[RAMHD_MAX_DEVICES] = { NULL, };
static RAMHD_DEV *rdev[RAMHD_MAX_DEVICES] = { NULL, };
...
static struct block_device_operations ramhd_ops = 
{
    .owner      = THIS_MODULE,
    .open       = ramhd_open,
...
static unsigned int ramhd_make_request(struct request_queue *q, struct bio *bio)
{
    char *pRHdata;
    char *pBuffer;
    struct bio_vec bvec;
    ...
    bio_for_each_segment(bvec, bio, bi)
    {
...
MODULE_AUTHOR("Gallen@Longsys");
MODULE_DESCRIPTION("what?");
MODULE_LICENSE("GPL");
